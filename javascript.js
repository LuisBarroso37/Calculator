class Calculator extends React.Component {
    ...
    render() {
      return (
        <div className="container-fluid calculator">
          <Display displayCalc={this.state.displayCalc} displayChar={this.state.displayChar}/>
          <Characters onClick={this.handleClick}/>
        </div>
      );
    }
  }
  ...
  const Characters = (props) => {
    const chars = ['(', ')', 'DEL', 'AC', '7', '8', '9', '/', '4', '5', '6', '*', '1', '2', '3', '-', '0', '.', '=', '+'];
      ...
      const listChars = chars.map((item, index) => {
          return (
            <li onClick={() => props.onClick(item)} id={writtenChars[index]}>{item}</li>
          ) 
      });
  ...
  const Display = (props) => {
    return (
      <div id='display'>
        <p id="d1">{props.displayCalc}</p>
        <p id="d2">{props.displayChar}</p>
      </div>
    );
  };
